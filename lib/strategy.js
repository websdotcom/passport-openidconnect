...
Strategy.prototype.authenticate = function (req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    this.configure(identifier, function (err, config) {
      ...
      params['state'] = stateInfo.generateState();
      ...
      if (params['state'] && options.response) {
        ...
        require('cookie-parser')()(req, options.response, function (err) {
          if (!err) {
            options.response.cookie(self._stateCookieName, stateInfo.generateCookieState(), {httpOnly: true});
          }
        });
      }
...
Strategy.prototype.stateInfo = function () {
  var algorithm = 'aes-256-ctr';
  var password = utils.uid(16);

  return {
    generateCookieState: function () {
      return password;
    },
    generateState: function () {
      var cipher = crypto.createCipher(algorithm, password);
      var crypted = cipher.update(Date.now().toString(), 'utf8', 'base64');
      crypted += cipher.final('base64');
      return crypted;
    },
    verifyState: function (cookieState, queryState) {
      try {
        var decipher = crypto.createDecipher(algorithm, cookieState);
        var dec = decipher.update(queryState, 'base64', 'utf8');
        dec += decipher.final('utf8');

        if (dec > Date.now() || dec < (Date.now() - this._stateTimeout * 1000)) {
          return false;
        } else {
          return true;
        }
